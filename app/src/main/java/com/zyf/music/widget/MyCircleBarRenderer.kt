...
class MyCircleBarRenderer (
        ...
        private var mCycleColor: Boolean = false)
        : IRenderer {
    enum class Type {
    ...
    private var mAngleModulation = 0.0f
    ...
    override fun calculate(drawArea: Rect, data: ByteArray) {
        ...
        for (i in 0 until data.size / divisions) {
            ...
            val cartPoint2 = when (type) {
                Type.TYPE_A -> floatArrayOf((i * divisions).toFloat() / (data.size - 1), drawHeight / 2f + dbValue-10f)
    ...
    private fun toPolar(cartesian: FloatArray, rect: Rect): FloatArray {
        ...
        var radius = (rect.width() / 2 * (1 - mAggresive) + mAggresive * cartesian[1] / 2) * (1 - modulationStrength + modulationStrength * (1 + Math.sin(mModulation)) / 2)
